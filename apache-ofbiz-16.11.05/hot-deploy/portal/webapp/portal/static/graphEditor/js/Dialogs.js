	...
	if (format == 'xml') {
		...
		if (param.length <= MAX_REQUEST_SIZE && w * h < MAX_AREA) {
			...
			var req = new mxXmlRequest(EXPORT_URL, 'format=' + format +
				...
				'&dpi=' + dpi);
			window.req = req;
			window.open("save.html?" + param, "_blank");
...
function loadDoc(data) {
	...
	// 	}
	// };
	// xhttp.open("POST", "save.html", false);
	// xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	// xhttp.send(data);

	// var req = new mxXmlRequest("save.html", data, 'POST', false);
	// req.send();
	...
	// new mxXmlRequest("http://localhost:5500/save.html", 'xml=' + data, "POST", false).send();
	//new mxXmlRequest("https://postb.in/1576466711632-4356219475157", 'xml=' + JSON.stringify(data), "POST", false).send();

}

// document.querySelector(".deploy").addEventListener('click', function () {
// 	// sendData(mxUtils.getXml(editorUi.editor.getGraphXml()));
// 	alert()
// });
...
function sendData(data) {
	...
	var xml = encodeURIComponent(data);
...
var EditDataDialog = function (ui, cell) {
	...
	var addRemoveButton = function (text, name) {
		...
		removeAttr.className = 'geButton deleteBtn';
		...
		removeAttr.style.display = 'block';
...
// var EditDataDialog2 = function (ui, cell) {
// 	var div = document.createElement('div');
// 	var graph = ui.editor.graph;
// 	// console.log(cell);
// 	var value = graph.getModel().getValue(cell);
...
// 		}
// 	}
// 	// temp.push({name: "Spider", value: "Man"});
...
var LayersWindow = function (editorUi, x, y, w, h) {
	...
	//Aditya
	var dataLink2 = link.cloneNode();
	dataLink2.innerHTML = '<div class="geSprite geSprite-dots" style="display:inline-block;"></div>';
	dataLink2.setAttribute('title', mxResources.get('defaultJsonData'));

	mxEvent.addListener(dataLink2, 'click', function (evt) {
		if (graph.isEnabled()) {
			editorUi.showDataDialog(selectionLayer);
		}
		mxEvent.consume(evt);
	...
	if (!graph.isEnabled()) {
		dataLink2.className = 'geButton mxDisabled';
	}

	ldiv.appendChild(dataLink2);
	//Aditya
	...
	function refresh() {
		...
		dataLink2.setAttribute('title', "Edit Patt Data");
